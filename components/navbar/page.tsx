            ...
            <Sheet open={showNavbar} onOpenChange={setShowNavbar}>
                <ul className="hidden h-28 items-center justify-around min-[780px]:flex">
                        ...
                        {navList.map(({ name, url }) => {
                            return (
                                <Link
                                    key={url}
                                    href={url}
                                    className={`group ${path === url ? "active" : ""}`}
                                >
                                    <p
                                        className={`${path === url
                                            ? "text-primary text-base font-semibold font-primary"
                                            : "font-[500]"
                                            }`}
                                    >
                                        {name}
                    ...
                    <li className="flex gap-4 max-[920px]:hidden">
                        ...
                        <button className="w-24 h-10 bg-primary text-lg rounded-xl text-white font-medium font-primary">
                            Log in
                        </button>
                    </li>
                ...
                <ul className="hidden max-[780px]:flex h-28 items-center justify-between mx-6">
                    <li className="flex gap-4">
                        <SheetTrigger onClick={() => setShowNavbar(true)} className="text-black text-4xl">
                            <PiListLight />
                        </SheetTrigger>
                        <Link href="/">
                            <Image src={logo} width={120} height={120} alt="Logo" />
                    ...
                    <li className="flex gap-4 max-[400px]:hidden">
                        <button className="w-10 h-10 flex justify-center items-center rounded-full bg-primary">
                            <Image src={cart_icon} width={24} height={24} alt="CART_IMAGE" />
                        </button>
                        <div className="w-24 h-10 flex items-center justify-center bg-primary text-lg rounded-xl text-white font-medium font-primary">
                            Log in
                        </div>
                    </li>
                    <li className="hidden max-[400px]:flex">
                        <button className="w-10 h-10 flex justify-center items-center rounded-full bg-primary">
                            <Image src={cart_icon} width={24} height={24} alt="CART_IMAGE" />
                        </button>
                    </li>
                    ...
                    <div>
                        <ul>
                            ...
                            <li className="flex flex-col items-start gap-6 mx-1 mt-6">
                                {navList.map(({ name, url }) => {
                                    return (
                                        <Link onClick={() => setShowNavbar(false)} key={url} className="w-fit" href={url}>
                                            <p
                                                className={`${path === url
